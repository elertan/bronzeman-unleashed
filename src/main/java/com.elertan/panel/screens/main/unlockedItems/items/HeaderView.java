...
public class HeaderView extends JPanel implements AutoCloseable {
    ...
    private static final Map<UnlockedItemsScreenViewModel.SortedBy, String> SORTED_BY_LABELS =
        ImmutableMap.<UnlockedItemsScreenViewModel.SortedBy, String>builder()
    ...
    private HeaderView(HeaderViewViewModel viewModel, BUResourceService buResourceService) {
        ...
        configButton.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) { configButton.setBackground(ColorScheme.DARK_GRAY_HOVER_COLOR); }
            @Override public void mouseExited(MouseEvent e) { configButton.setBackground(ColorScheme.DARKER_GRAY_COLOR); }
        });
        ...
        sortedByComboBoxBinding = Bindings.bindComboBox(sortedByComboBox,
            new Property<>(new ArrayList<>(SORTED_BY_LABELS.keySet())),
            viewModel.sortedBy, new Property<>(SORTED_BY_LABELS));
        ...
        Property<List<Long>> unlockedByOptions = viewModel.accountHashesFromAllUnlockedItems.derive(list -> {
            List<Long> result = new ArrayList<>(list.size() + 1);
            result.add(null);
            result.addAll(list);
            return result;
        });
        Property<Map<Long, String>> unlockedByLabels = viewModel.accountHashToMemberNameMap.derive(map -> {
            Map<Long, String> result = new HashMap<>(map);
            result.put(null, "Everyone");
            return result;
        });
        unlockedByComboBoxBinding = Bindings.bindComboBox(
            unlockedByComboBox, unlockedByOptions, viewModel.unlockedByAccountHash, unlockedByLabels);
